...
public class StartupState implements SystemStateInterface {
    ...
    // private static long startupConnectionTimePeriod = 10000; //10 seconds for
    // testing version
    ...
    // flag is starting early before 10 minute timer or before all supposed to be
    // clients have joined
    ...
    public boolean performOperation() {
        // Check if the required number of trains, stations, and checkpoints are
        // connected
    ...
    private void checkReadyToMap(int curTrains, int curStations, int curCheckpoints) throws Exception {
        if ((curTrains == maxTrains && curStations == maxStations && curCheckpoints == maxCheckpoints) || // If all
                                                                                                          // clients
                                                                                                          // have
                                                                                                          // connected
                (System.currentTimeMillis() - timeOnStart >= startupConnectionTimePeriod) || // If the timeout of 10
                                                                                             // minutes has occured
    ...
    private class CurrentTrainInfo {
        ...
        private void stopTrainAtCheckpoint(int zone) {
            ...
            String message = MessageGenerator.generateExecuteMessage("ccp", train.id, System.currentTimeMillis(),
                    SpeenEnum.STOP);
            train.sendMessage(message);
