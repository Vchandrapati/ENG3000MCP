...
public class WaitingState implements SystemStateInterface {
    ...
    private static final int MAX_CHECK_STATIONS = 10;
    ...
    public boolean performOperation () {
        ...
        // Checks if the timeout has occurred or if the correct amount of clients is joined
        if (elapsedTime >= STARTUP_CONNECTION_TIME_PERIOD
                || (db.getBladeRunnerCount() == MAX_BLADERUNNERS
                && db.getCheckpointCount() + db.getStationCount() == MAX_CHECK_STATIONS)) {
            logger.log(Level.INFO, "Timeout state has occurred");
            return true;
        }
        return false;
    }
