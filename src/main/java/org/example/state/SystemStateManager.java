...
public class SystemStateManager {
    ...
    /**
     * Provides the singleton instance of SystemStateManager.
    ...
    public void handleTrip(TripEvent event) {
        ...
        if (currentState == SystemState.MAPPING) {
            MappingState.addTrip(event.getLocation(), event.isUntrip());
            logger.log(Level.INFO, "System state manager has detected untrip {0}", event.getLocation());
        }
    }
    ...
    public void addUnresponsiveClient(ClientErrorEvent event) {
        if (currentState != SystemState.WAITING && event.getId() != null && event.getReason() != null
                && db.addUnresponsiveClient(event.getId(), event.getReason())) {
            logger.log(Level.WARNING, "Client {0} has {1}", new Object[] {event.getId(), event.getReason()});
            error = true;
        }
    }
}
