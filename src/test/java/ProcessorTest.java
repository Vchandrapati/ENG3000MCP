...
// private Database mockDb;private BladeRunnerClient mockBladeRunnerClient;private
// SystemStateManager mockSystemStateManager;private Processor mockProcessor;private
// MessageGenerator mockMessageGenerator;private EventBus mockEventBus;private Processor
// processor;private TestLogHandler logHandler; // Custom
// // log
// // handler
// // to
// // capture
// // log
// // messages
// private static final Logger logger=Logger.getLogger(Logger.GLOBAL_LOGGER_NAME);TripEvent
// tripEventTripped;TripEvent tripEventUntripped;


// @BeforeEach void setUp(){
// // Mocking the Database and BladeRunnerClient
// mockDb=mock(Database.class);mockSystemStateManager=mock(SystemStateManager.class);mockMessageGenerator=mock(MessageGenerator.class);mockBladeRunnerClient=mock(BladeRunnerClient.class);mockEventBus=mock(EventBus.getInstance().getClass());tripEventTripped=new
// TripEvent(3,false);tripEventUntripped=new TripEvent(3,true);

// // Set up the Processor with the mocked Database
// processor=new Processor(mockEventBus,mockDb);>>>>>>>d7252bf48c45153b21d70522f64d8e16fc72cbb6
...
// ArgumentCaptor<ClientErrorEvent>checkpointTrippedAccess(int checkpointTripped,boolean
// untrip,SystemState state)throws Exception{
// // Use reflection to access the private checkpointTripped method
// Method
// method=Processor.class.getDeclaredMethod("checkpointTripped",TripEvent.class);method.setAccessible(true);
// // Allow
// // access
// // to
// // the
...
// Method
// updateStateMethod=Processor.class.getDeclaredMethod("updateState",StateChangeEvent.class);updateStateMethod.setAccessible(true);
// // Allow
// // access
// // to
// // the
...
// StateChangeEvent stateChangeEvent=new
// StateChangeEvent(state);updateStateMethod.invoke(processor,stateChangeEvent);
...
// ArgumentCaptor<ClientErrorEvent>handleTripAccess(int checkpointTripped,int previousBlock,boolean
// untrip)throws Exception{
// // Use reflection to access the private handleTrip method
// Method
// method=Processor.class.getDeclaredMethod("handleTrip",int.class,int.class,boolean.class);method.setAccessible(true);
// // Allow
// // access
// // to
// // the
...
// ArgumentCaptor<ClientErrorEvent>reverseTripAccess(BladeRunnerClient reversingBladeRunner,int
// checkpointTripped,boolean untrip)throws Exception{
// // Use reflection to access the private reverseTrip method
// Method
// method=Processor.class.getDeclaredMethod("reverseTrip",BladeRunnerClient.class,int.class,boolean.class);method.setAccessible(true);
// // Allow
// // access
// // to
// // the
...
// Method
// method=Processor.class.getDeclaredMethod("bladeRunnerStopped",BladeRunnerStopEvent.class);method.setAccessible(true);
// // Allow
// // access
// // to
// // the
...
// BladeRunnerStopEvent event=new BladeRunnerStopEvent(id);method.invoke(processor,event);

// // Capture the event to verify later
// ArgumentCaptor<BladeRunnerStopEvent>captor=ArgumentCaptor.forClass(BladeRunnerStopEvent.class);verify(mockEventBus).publish(captor.capture());
// // Verify
// // that
// // the
// // publish
...
// =======
// // CHECKPOINT TRIPPED 100% coverage
// @Test void testCheckPointTrippedInvalidTrip()throws
// Exception{ArgumentCaptor<ClientErrorEvent>captor=checkpointTrippedAccess(3,true,SystemState.RUNNING);

// verify(mockEventBus).publish(captor.capture());

// // Extract the captured argument and assert
// ClientErrorEvent
// capturedEvent=captor.getValue();assertEquals("CP03",capturedEvent.getId());assertEquals(ReasonEnum.INCORTRIP,capturedEvent.getReason());
...
// @Test void testCheckPointTrippedBladeRunnerMissing()throws Exception{CheckpointClient
// cc=mock(CheckpointClient.class);

// // Mock the database behavior
// when(mockDb.getBlockCount()).thenReturn(10);when(mockDb.getClient("CP03",CheckpointClient.class)).thenReturn(Optional.of(cc));
...
// // assertTrue(logHandler.containsLog(Level.WARNING, "Tried to get blade runner at checkpoint {0}
// but
// // failed"));
// // }
...
// @Test void testCheckPointTrippedSendToMapping()throws Exception{
// // Arrange
// CheckpointClient cc=mock(CheckpointClient.class);
...
// @Test void testCheckPointTrippedReversing()throws Exception{CheckpointClient
// cc=mock(CheckpointClient.class);BladeRunnerClient br=mock(BladeRunnerClient.class);

// // Mock the database behavior
// when(mockDb.getBlockCount()).thenReturn(10);when(mockDb.getClient("CP03",CheckpointClient.class)).thenReturn(Optional.of(cc));


// when(mockDb.getLastBladeRunnerInBlock(3)).thenReturn("BR01");when(mockDb.getClient("BR01",BladeRunnerClient.class)).thenReturn(Optional.of(br));


// when(br.getStatus()).thenReturn(MessageEnums.CCPStatus.RSLOWC);when(mockDb.isBlockOccupied(3)).thenReturn(true);


// ArgumentCaptor<ClientErrorEvent>captor=checkpointTrippedAccess(3,true,SystemState.RUNNING);

// // Verify that the event was published, no need to assert for an INCORTRIP since we're in the
// // reversing logic
// assertTrue(logHandler.containsLog(Level.FINEST,"Reversing reached"));

// }

// @Test void testCheckPointTrippedForward()throws Exception{CheckpointClient
// cc=mock(CheckpointClient.class);BladeRunnerClient br=mock(BladeRunnerClient.class);
...
// when(br.getStatus()).thenReturn(MessageEnums.CCPStatus.FFASTC);when(mockDb.isBlockOccupied(3)).thenReturn(true);
...
// @Test void testHandleTripEmptyBR()throws Exception{
...
// @Test void testHandleTripTrippedBlockFull()throws Exception{CheckpointClient
// cc=mock(CheckpointClient.class);BladeRunnerClient
// br=mock(BladeRunnerClient.class);when(br.getId()).thenReturn("BR01");

// // Mock the database behavior
// when(mockDb.getLastBladeRunnerInBlock(2)).thenReturn("BR01");when(mockDb.getClient("BR01",BladeRunnerClient.class)).thenReturn(Optional.of(br));when(mockDb.isBlockOccupied(3)).thenReturn(true);
...
// @Test void testHandleTripCheckpointStation(){CheckpointClient
// cc=mock(CheckpointClient.class);StationClient st=mock(StationClient.class);BladeRunnerClient
// br=mock(BladeRunnerClient.class);

// when(mockDb.getLastBladeRunnerInBlock(2)).thenReturn("BR01");when(mockDb.getClient("BR01",BladeRunnerClient.class)).thenReturn(Optional.of(br));when(mockDb.isBlockOccupied(3)).thenReturn(false);>>>>>>>d7252bf48c45153b21d70522f64d8e16fc72cbb6
...
// // }
// =======when(mockDb.getClient("ST04",StationClient.class)).thenReturn(Optional.of(st));when(mockDb.getStationIfExist(4)).thenReturn(Optional.of(st));when(mockBladeRunnerClient.isDockedAtStation()).thenReturn(false);

// processor.handleTrip(3,2,true);>>>>>>>d7252bf48c45153b21d70522f64d8e16fc72cbb6
...
// @Test void testHandleTripNextBlockOccupied()throws Exception{CheckpointClient
// cc=mock(CheckpointClient.class);StationClient st=mock(StationClient.class);BladeRunnerClient
// br=mock(BladeRunnerClient.class);

// when(mockDb.getLastBladeRunnerInBlock(2)).thenReturn("BR01");when(mockDb.getClient("BR01",BladeRunnerClient.class)).thenReturn(Optional.of(br));when(mockDb.isBlockOccupied(3)).thenReturn(false);when(mockDb.getBlockCount()).thenReturn(10);when(mockDb.isBlockOccupied(4)).thenReturn(true);when(mockDb.getClient("ST04",StationClient.class)).thenReturn(Optional.of(st));
...
// // Extract the captured argument and assert its properties
// ClientErrorEvent capturedEvent=captor.getValue();assertEquals("BR01",capturedEvent.getId()); //
// Ensure
// // the
// // expected
// // ID
...
// @Test void testReverseTripBadReverse()throws Exception{BladeRunnerClient
// br=mock(BladeRunnerClient.class);>>>>>>>d7252bf48c45153b21d70522f64d8e16fc72cbb6
...
// @Test void testReverseTripBRInPreviousBlock()throws Exception{StationClient
// sc=mock(StationClient.class);CheckpointClient cc=mock(CheckpointClient.class);BladeRunnerClient
// br=mock(BladeRunnerClient.class);


// when(mockDb.getStationIfExist(3)).thenReturn(Optional.of(sc));when(br.getStatus()).thenReturn(MessageEnums.CCPStatus.FFASTC);when(mockDb.getBlockCount()).thenReturn(10);when(mockDb.getClient("CP03",CheckpointClient.class)).thenReturn(Optional.of(cc));when(mockDb.getClient("ST03",StationClient.class)).thenReturn(Optional.of(sc));when(mockDb.isBlockOccupied(2)).thenReturn(true);when(mockDb.getClient("CP02",CheckpointClient.class)).thenReturn(Optional.of(cc));

// ArgumentCaptor<ClientErrorEvent>captor=reverseTripAccess(br,3,false);

// verify(mockEventBus).publish(captor.capture());

// assertTrue(logHandler.containsLog(Level.WARNING,"blade Runner reversing but previous block
// occupied"));}


// // bladeRunner stopped

// @Test void testBladeRunnerStoppedWithPresentClient()throws Exception{BladeRunnerClient
// br=mock(BladeRunnerClient.class);StationClient sc=mock(StationClient.class);

// // Mocking necessary database interactions
// when(mockDb.getClient("BR01",BladeRunnerClient.class)).thenReturn(Optional.of(br));when(mockDb.getStationIfExist(1)).thenReturn(Optional.of(sc));
// // Assuming
// // calculateNextBlock
// // returns
// // 1
...
// // this
// // example
// when(mockDb.getBlockCount()).thenReturn(10);when(mockDb.getClient("STA02",StationClient.class)).thenReturn(Optional.of(sc));when(br.getZone()).thenReturn(1);
// // Adjust
// // the
// // return
// // value
// // as
// // necessary
// when(mockDb.getStationIfExist(2)).thenReturn(Optional.of(sc));when(sc.getId()).thenReturn("STA02");>>>>>>>d7252bf48c45153b21d70522f64d8e16fc72cbb6
...
// // public void flush() {
// // }
// =======ArgumentCaptor<BladeRunnerStopEvent>captor=bladeRunnerStoppedAccess("BR01");>>>>>>>d7252bf48c45153b21d70522f64d8e16fc72cbb6
...
// // }
// // }
// =======verify(br).sendExecuteMessage(MessageEnums.CCPAction.STOPO);verify(br).updateStatus(MessageEnums.CCPStatus.STOPO);


// verify(sc).sendExecuteMessage(MessageEnums.STCAction.OPEN);verify(sc).updateStatus(MessageEnums.STCStatus.ONOPEN);}

// @Test void testBladeRunnerStoppedWithAbsentClient()throws Exception{String id="BR02";
...
// verify(mockDb,times(1)).getClient(id,BladeRunnerClient.class);verify(mockEventBus,never()).publish(any());}
